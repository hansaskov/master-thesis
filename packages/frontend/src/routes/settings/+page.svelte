<script lang="ts">
    ...
    $: usersAndInvites = users.map(user => ({
        ...user,
        onboardingStatus: onboardingInvitations.find(invite => invite.email === user.email)?.status || 'Not Invited'
    }));
    ...
    function addUser() {
        if (newUserEmail) {
            users = [...users, { id: users.length + 1, name: '', email: newUserEmail, role: 'User', avatarUrl: '' }];
            onboardingInvitations = [...onboardingInvitations, { email: newUserEmail, status: 'pending' }];
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <Table.Cell class="text-right">
                                    <DropdownMenu.Root>
                                        ...
                                        <DropdownMenu.Content align="end">
                                            ...
                                            {/if}
                                            <DropdownMenu.Separator />
                                            <DropdownMenu.Item on:click={() => removeUser(person.id)} class="text-red-600">
                                                Remove User
                                            </DropdownMenu.Item>
                ...
                <div class="mt-6 space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-muted-foreground">Receive email notifications for activity in your organization</p>
                        </div>
                        <Switch.Root checked={organizationSettings.notificationEmails} on:click={() => toggleSetting('notificationEmails')} />
                    </div>
                    <div class="flex items-center justify-between">
                        <div>
                            <Label>Public Profile</Label>
                            <p class="text-sm text-muted-foreground">Allow your organization profile to be publicly visible</p>
                        </div>
                        <Switch.Root checked={organizationSettings.publicProfile} on:click={() => toggleSetting('publicProfile')} />
                    </div>
                </div>
            </Card.Content>
        </Card.Root>
        ...
        <Card.Root>
            ...
            <Card.Content>
                        ...
                        <Table.Body>
                            {#each $productionSystems as system}
                                <Table.Row>
                                    ...
                                    <Table.Cell class="text-right">
                                        <Button variant="destructive" size="sm" on:click={() => removeProductionSystem(system)}>
                                            Remove
                                        </Button>
                                    </Table.Cell>
                                </Table.Row>
                ...
                {:else}
                    <p class="text-center text-muted-foreground mt-4">No production systems added yet.</p>
                {/if}
            </Card.Content>
        </Card.Root>
    </div>
