...
describe("system model queries", async () => {
	it("select all parts for system model", async () => {
		const system_model = await Queries.systemModels.selectOnId({
			id: "Dgw8Rvd4QoeC",
		});
		...
		const relationship1 = await Queries.partsToSystemModels.create({
			part_id: part.id,
			system_model_id: system_model[0].id,
		});
		const relationship2 = await Queries.partsToSystemModels.create({
			part_id: part2.id,
			system_model_id: system_model[0].id,
		});
