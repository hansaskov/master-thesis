...
mod monitoring {
    // Provide a stub or alternative implementation for non-Windows platforms
    use crate::module::{Module, ModuleCtx};
    use anyhow::Result;
    use serde::{Deserialize, Serialize};
    ...
    impl Monitoring {
        pub fn new(ctx: ModuleCtx, _sensors_config: Vec<SensorConfig>) -> Self {
            Self { ctx }
        }
    }
    ...
    impl Module for Monitoring {
        async fn run(&mut self) -> Result<()> {
            self.ctx.send_log("Monitoring module is only available on Windows".to_string());
            Ok(())
        }
    }
}

// Re-export the functionality so it can be used uniformly
pub use monitoring::*;